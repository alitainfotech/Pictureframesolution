    ...
    */
    ...
    'events' => [
        ...
        'beforeRenderTheme' => function (Theme $theme)
        {
            ...
            $theme->asset()->container('footer')->add('gsap', 'https://unpkg.co/gsap@3/dist/gsap.min.js', ['jquery']);
            ...
            $theme->asset()->container('footer-local')->usePath()->add('scripts', 'js/scripts.js', ['jquery'], [], $version);
            $theme->asset()->container('footer-local')->usePath()->add('backend-js', 'js/backend.js', ['jquery'], [], $version);


            $theme->asset()->container('local-header')->usePath()->add('style', 'css/style.css', [], [], $version);
            if (BaseHelper::siteLanguageDirection() == 'rtl') {
                $theme->asset()->container('local-header')->usePath()->add('rtl-style', 'css/rtl-style.css', [], [], $version);
            }

            if (function_exists('shortcode')) {
                $theme->composer(['page', 'post', 'ecommerce.product'], function (\Botble\Shortcode\View\View $view) use ($theme, $version) {
                    $theme->asset()->container('footer-local')->usePath()->add('app-js', 'js/app.js', ['jquery', 'carousel-js'], [], $version);
                    $view->withShortcodes();
                });
            }
