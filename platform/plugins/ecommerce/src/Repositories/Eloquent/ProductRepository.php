...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function getProductsWithCategory(array $params)
    {
        ...
        $this->model = $this->model
            ...
            ->leftJoin('ec_product_categories as base_category', 'base_category.id', '=',
            ...
            ->where(function ($query) use ($params) {
                ...
                if ($params['categories']['by'] == 'id') {
                    return $query
                        ->whereIn('ec_product_category_product.category_id', $params['categories']['value_in']);
                }
                return $query
     ...
     * {@inheritDoc}
     */
    public function getFrameProducts(array $params = [])
    {
        ...
        $this->model = $this->model
            ->leftJoin('ec_product_category_product', 'ec_products.id', '=', 'ec_product_category_product.product_id')
            ->leftJoin('ec_product_categories', 'ec_product_categories.id', '=', 'ec_product_category_product.category_id')
            ->distinct()
            ->where(function ($query) use ($params) {
                /**
                 * @var Builder $query
                ...
                return $query
                    ->whereIn('ec_product_categories.' . $params['categories']['by'], $params['categories']['value_in']);
            });

        $products = $this->advancedGet($params)->groupBy('category_id');

        for($i = 1; $i <= count($products); $i++){
          for($j = 0; $j < count($products[$i]); $j++)
          {
              $configurableProductId = $products[$i][$j]->id;

              $productVariations = app(ProductVariationInterface::class)->allBy([
                  'configurable_product_id' => $configurableProductId,
              ]);

              $products[$i][$j]->product_image = $this->getProductImage($products[$i][$j]->images);
              unset($products[$i][$j]->images);

              $productVariationsInfo = app(ProductVariationItemInterface::class)
                  ->getVariationsInfoDetails($productVariations->pluck('id')->toArray());

              for($k = 0; $k < count($productVariationsInfo); $k++){
                  $productVariationsInfo[$k]->attribute_image = get_image_url($productVariationsInfo[$k]->attribute_image);
                  $productVariationsInfo[$k]->product_image = $this->getProductImage($productVariationsInfo[$k]->product_images);
                  unset($productVariationsInfo[$k]->product_images);
              }

              $products[$i][$j]->variations = $productVariationsInfo;
    ...
    {
      $product_images = (is_string($images))? json_decode($images, true) : $images;
      for($i = 0; $i < count($product_images); $i++)
        if(!empty($product_images[$i])) return get_image_url($product_images[$i]);

      return RvMedia::getImageUrl(RvMedia::getDefaultImage(), 'thumb', false, RvMedia::getDefaultImage());
    }
}
