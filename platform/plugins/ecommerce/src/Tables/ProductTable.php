...
class ProductTable extends TableAbstract
{
    ...
    public function columns()
    {
        return [
            ...
            'type'         => [
                'title' => trans('plugins/ecommerce::products.type'),
                'class' => 'text-left',
            ],
            'name'         => [
    ...
    protected function addCreateButton(string $url, $permission = null, array $buttons = []): array
    {
        if (!$permission || Auth::user()->hasPermission($permission)) {
            ...
            $buttons['create'] = [
                'link' => $url,
                'text' => '<i class="fa fa-plus"></i> ' . trans('plugins/ecommerce::products.create_regular'),
            ];

            $buttons['create2'] = [
                'link' => route('products.create-fps'),
                'text' => '<i class="fa fa-plus"></i> ' . trans('plugins/ecommerce::products.create_framebuilder'),
            ];
        }
