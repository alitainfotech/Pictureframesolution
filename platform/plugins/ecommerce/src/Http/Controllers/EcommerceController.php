...
use Botble\Ecommerce\Services\StoreProductSizesService;
...
class EcommerceController extends BaseController
{
     ...
     * @return Factory|View
     */
    public function getSettingsSizes( ProductSizeInterface $productSizeInterface )
    {
        page_title()->setTitle(trans('plugins/ecommerce::framebuilder.sizes'));

        Assets::addScripts(['jquery-ui'])
            ->addScriptsDirectly([
                'vendor/core/plugins/ecommerce/js/sizes.js',
            ])
            ->addStylesDirectly([
                'vendor/core/plugins/ecommerce/css/ecommerce.css',
                'vendor/core/plugins/ecommerce/css/sizes.css',
            ]);
        ...
        return view('plugins/ecommerce::settings.sizes', compact('sizes'));
    }

    /**
     * @return Factory|View
     */
    public function postSettingsSizes(
      Request $request,
      ProductSizeInterface $productSizeInterface,
      StoreProductSizesService $service,
      BaseHttpResponse $response )
    {

      $sizes = json_decode($request->input('product_sizes'), true) ?: [];

      if (!$sizes) {
          return $response
              ->setNextUrl(route('ecommerce.sizes'))
              ->setError()
              ->setMessage(trans('plugins/ecommerce::sizes.require_at_least_one_size'));
      }

      $deletedSizes = json_decode($request->input('deleted_product_sizes', []), true) ?: [];

      $service->execute($sizes, $deletedSizes);
