...
$(function () {
  ...
  $(document).on('submit', '.form-import-data', function (event) {
    ...
    var failureTempalte = $('#failure-tempalte').html();
    $.ajax({
      ...
      success: function success(data) {
        ...
        if (data.data.failures) {
          _.map(data.data.failures, function (val) {
            result += failureTempalte.replace('__row__', val.row).replace('__attribute__', val.attribute).replace('__errors__', val.errors.join(', '));
          });
        }
