...
var EcommerceProductAttribute = /*#__PURE__*/function () {
  ...
  _createClass(EcommerceProductAttribute, [{
    ...
    value: function addNewAttribute() {
      ...
      var template = _self.template.replace(/__id__/gi, 0).replace(/__position__/gi, 0).replace(/__checked__/gi, _self.totalItem == 0 ? 'checked' : '').replace(/__title__/gi, '').replace(/__description__/gi, '').replace(/__slug__/gi, '').replace(/__color__/gi, '').replace(/__image__/gi, '');
    ...
    value: function exportData() {
      ...
      $('.swatches-container .swatches-list li').each(function (index, item) {
        ...
        data.push({
          ...
          description: $current.find('.swatch-description input').val(),
